...
13. **同じキャラの衣装違いVRM複数体に、あとから肌マテリアルだけ反映したいことがある。**
    （例: しずくの体に光沢のあるmatcapを追加、既存4衣装すべてに反映）
    衣装ごとに完全に別のVRMファイルなので、実行時にランダムアクセスで
    差し替えるより、VRMファイル自体を1回だけ書き換える方が本筋
    （余計なpropsの配線やfetchが増えない）。`scripts/patch-skin-material.py`で、
    参照VRMの`Body_00_SKIN`・`Face_00_SKIN`マテリアルとその使用テクスチャを
    対象VRMのバッファ末尾に追記し、対象側の同名マテリアルをまるごと
    差し替える（既存のメッシュ・アクセサ・他マテリアルのインデックスは
    一切変更しないので服・髪・顔には影響しない。元の肌テクスチャは
    未参照のまま残るぶんファイルサイズはやや増える）。出力は必ず一旦
    別ファイルに書き、`materials`件数・`buffers[0].byteLength`と実際の
    バッファ長の一致・ブラウザでの見た目を確認してから対象ファイルへ
    上書きすること。
