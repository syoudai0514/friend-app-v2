...
9. **別キャラの衣装は「服（`_CLOTH`）だけ」を借りる。体は絶対に借りない。**
   ...
   **【重要・訂正】「Body_00_SKINは下着まで焼き込まれた完全な素体」は誤り
   だった。** VRoidは服の下に隠れる体をジオメトリではなく**テクスチャの
   アルファ**で消しており（`Body_00_SKIN`は`alphaMode=MASK`）、削り具合は
   キャラ・衣装ごとに全く違う。実測した透明率:
   aimi/knit・swimsuit＝0%、shizuku/fftifa・knit・leather＝0%（完全）、
   shizuku/casual＝7.3%、rena（全衣装）＝13.0%（バスト位置は94%）、
   nagi/default＝24.6%、aimi/shirt＝49.6%。自分の衣装のうちはその服が
   欠損部をちょうど覆うので見えないが、借り物の服は覆う範囲が違うため
   露出し、ギザギザの穴として見える（なぎ・れなで顕著。20番参照）。
   **アイミー・しずくの一部を目視確認しただけで「4体すべてで確認済み」と
   書いてしまったのが敗因。キャラのVRMが増えたら、服だけ隠すのに加えて
   Body_00_SKINベーステクスチャのアルファチャンネルを画像として見て、
   透明領域が無いか確認すること**（`scripts/build-complete-skins.py`の
   `clear_ratio()`と同じ要領。一時的なデバッグページで`hideClothes`だけ
   立てて描画するのも有効だが、それだけでは透明→黒（RGB=0,0,0）の穴を
   見落とすことがある）。
...
21. **【解決済み・経緯として保存】借りた服は提供元の体型に合わせた硬い
    メッシュなので、着る側の方が大きい部位は必ず服の外へ出る
    （しずくの胸・肩・背中が服を突き抜ける）。**（2026-08-06、実機で発覚）
    最初は`fit-clothes.ts`の`fitClothingToBody()`で、服のメッシュを
    体からのはみ出しが無くなる位置まで法線方向へ押し出す方式を試したが、
    **服の面は平らなので、頂点を体表の外まで動かしても面の内側が胸のような
    曲率の大きい部位を貫いたままになり**、押し出し量を上げると今度は
    服のメッシュ自体が破綻して黒い塊が出た（実際に何度も往復して確認）。
    最終的に9番のとおり「押し出す」のをやめ、`maskBodyUnderClothing()`で
    **VRoid自身と同じやり方＝服が覆っている範囲の体を隠す**方式に変えて
    構造的に解消した（詳細は22番）。
22. **なぎ・れなの体テクスチャがVRoid標準のアルファ削りで欠損しており
    （19番参照）、借り物の服を着ると欠損部が黒い穴として露出する事故が
    あった。修正は2段構え。**（2026-08-06〜07、なぎ×アイミーのオフショル
    ニットで実機発覚。ユーザー報告のスクショで判明）
    - **穴埋め**: `scripts/build-complete-skins.py`で、4キャラ共通のUV
      レイアウトを利用し、完全なテクスチャ（`shizuku/knit`）の同じUV位置
      から欠損部だけ埋めた`public/skin/<personaId>.webp`を生成。借り物の
      服を着ているときだけ`VrmModel.tsx`で`Body_00_SKIN`のテクスチャを
      これに差し替える。土台に選ぶ衣装は透明率だけで自動選択すると
      失敗する——しずくは`leather`（黒いレオタード状の面がBody側に
      焼き込まれている）が自動選択されてしまい、何を借りても脚が
      黒くなった。**衣装が体テクスチャに焼き込まれていないか必ず目視
      確認し、`PREFERRED_BASE`で明示指定すること。**
    - **貫通対策**: 穴を塞いだだけでは、露出した体が服の下から突き抜けて
      見える問題が残った。21番のとおり押し出しでは解決できなかったため、
      `fit-clothes.ts`の`maskBodyUnderClothing()`で**服が覆っている範囲の
      体そのものを隠す**方式にした（VRoid自身のアルファ削りを借り物の服の
      形に合わせて実行時にやり直すイメージ）。服の三角形を一様グリッドに
      登録し、体側の各頂点について最近傍の服面までの距離を測り、閾値
      （11cm）以内なら隠す。**襟ぐり・袖口・裾など服の開いた縁
      （1枚の三角形にしか属さない辺＝boundary edge）に接する三角形は
      判定から除く**——ここを含めると、服が終わっていて本来肌が見える
      べき場所（脇・裾のすぐ下）まで削れて別の黒い欠けを作る（実際に
      1度やって裾の下が黒く欠けた）。縁を除いた結果、襟ぐり際に体が
      1列だけ残ってギザギザの細い線として見える副作用が残るが、
      「服の縁の下へ1列だけ広げて隠す」対策も試した所、今度は裾のような
      別の縁で同じ理由で害の方が大きく出たため、**現状は襟ぐり際の
      細いギザつきを許容する判断で止めている**（大きな黒い塊よりは軽微）。
      改善の余地があるなら、境界三角形の判定を「開いた縁の種類」
      （襟ぐり/袖口/裾）で区別し、裾側だけディレーションを効かせない
      といった作り込みが必要。
