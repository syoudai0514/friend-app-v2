...
14. **肌の色・光沢は「キャラ全員が選べる」形にするなら、VRMファイルを個別に
    ...
    - **光沢**: しずくの肌はVRMファイル自体にmatcap＋parametric
      ...
      （ユーザーの要望が「脚の光沢」だったため）。
15. **借り物衣装（`Look.outfit`）試着中、本人のBodyを丸ごと非表示にすると
    首元に隙間ができる。**（2026-08-05）本人の頭（Face_00_SKINは両面描画）
    のすぐ下は本来Body_00_SKINの首が隠しているが、`hideBody=true`で
    Bodyマテリアルごと消すと、頭の下側の裏面が透けて暗い切り株のように
    見える事故になった（衣装元の体をどれだけ正確に位置合わせしても、
    別メッシュ同士である以上わずかな隙間は必ず残るため、位置合わせでは
    根本解決しない）。衣装元側で使っている「headボーンへの追従度で
    三角形をふるい分ける」処理（`filterBodyGeometryByHead`、旧
    `bodyGeometryWithoutHead`）を本人側にも反転して適用し（`keep:
    "onlyHead"`）、Bodyマテリアルは非表示にせず、首まわり（headボーンに
    強く追従する三角形）だけ本人自身のジオメトリを残すようにした。
    衣装元は`excludeHead`（閾値0.35）で頭側を削り、本人側は`onlyHead`
    （閾値0.12、わずかに低くして重ねる）で首側を残す非対称な閾値に
    することで、境目にすき間が残らないようにしている。衣装元の位置合わせは
    足元（minY）基準のまま（後述17番の理由で、headボーン基準への変更は
    一度試して元に戻した）。
...
17. **15番の首の隙間対策として、衣装元の位置合わせを足元基準から
    headボーンのワールド座標基準に変えたところ、別の事故を起こした。**
    （2026-08-05、しずく×なぎの服の組み合わせで発覚。ユーザーからの
    スクリーンショット3枚で判明）headボーンのワールド座標は
    `getWorldPosition()`で取得した「計測した一瞬の姿勢」の値であり、
    アイドルモーションの向き・揺れで常に動いている。これを全身の
    固定オフセット（`vrm.scene.position`）の計算に使うと、計測タイミングと
    実際の姿勢がズレたときに首が異常に伸びる事故になった（実機で複数枚
    確認された）。**姿勢で動く値を、姿勢に依存しないはずの固定オフセットの
    根拠に使ってはいけない**という教訓。位置合わせは足元（minY）基準に
    戻し、首の隙間対策は15番のジオメトリ差し替えだけに任せている
    （髪の位置合わせ`hairOffsetX/Y/Z`は同じheadボーン方式だが、髪は
    全身位置合わせと違い多少のズレが目立ちにくいため、今のところ
    問題は報告されていない。ただし同種のリスクを抱えていることに注意）。
18. **肌の色ピッカー（14番）は本人の肌色を変える際、Body_00_SKINだけを
    塗り替えてFace_00_SKINを塗り替えていなかったため、顔と体が別の色に
    分かれて見える事故になった。**（2026-08-05、なぎの肌色を変えて発覚。
    17番と同じユーザー報告で判明）`recolorBodyTexture`を適用する対象の
    マテリアル判定に`Face_00_SKIN`も加えて修正。衣装元側（借り物の体）に
    適用したときは顔がそもそも非表示なので実害はない。
