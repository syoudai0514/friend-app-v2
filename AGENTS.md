...
15. **借り物衣装（`Look.outfit`）試着中、本人のBodyを丸ごと非表示にすると
    ...
    することで、境目にすき間が残らないようにしている。衣装元の位置合わせも
    元は足元（minY）基準だったが、全身の高さだけを合わせても体型比が
    違うキャラ同士では首の位置がズレるため、髪の位置合わせ
    （`hairOffsetX/Y/Z`）と同じ「headボーンのワールド座標を合わせる」
    方式に変更した（裾が多少浮く／めり込むのは既知の許容範囲として残る）。
